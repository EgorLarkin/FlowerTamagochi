...
final class ChatViewModel: ObservableObject {
    ...
    private func readFlowerName() -> String {
        ...
        guard let text = try? String(contentsOf: fileURL).trimmingCharacters(in: .whitespacesAndNewlines),
              !text.isEmpty
        else { return "Цветок" }
...
struct ChatView: View {
    ...
    var body: some View {
        ...
        .onChange(of: bluetoothManager.isConnected) { if !$0 { dismiss() } }
    }
...
struct MessageBubble: View {
    ...
    private var bubble: some View {
        Text(message.content)
            ...
            .overlay(bubbleTail, alignment: isUser ? .bottomTrailing : .bottomLeading)
    ...
    // MARK: Tail (speech arrow)

    private var bubbleTail: some View {
        Triangle()
            .fill(bubbleBackground)
            .frame(width: 12, height: 12)
            .rotationEffect(.degrees(isUser ? -45 : 135))
