...
final class BluetoothManager: NSObject, ObservableObject {
    ...
    private var historyIsRead: Bool = false
    ...
    private func parseSensorData(_ message: String) {
        ...
        if historyIsRead {
            ...
            if parts.count > 0 {
                ...
                newHumidity = parseHumidity(from: dhtPart) ?? newHumidity
            }
            ...
            writeNewData(history: message)
            let newCount = readCount() + message.components(separatedBy: "\n").count
            writeNewCount(count: newCount)
            self.historyIsRead = true
        }
    }
    
    private func parseHumidity(from dhtPart: String) -> Float? {
...
extension BluetoothManager: CBPeripheralDelegate {
    ...
    private func writeNewData(history: String) {
    ...
    private func writeNewCount(count: Int) {
        let documentsURL = FileManager.default.urls(for: .documentDirectory, in: .userDomainMask).first!
        let fileURL = documentsURL.appendingPathComponent(self.deviceName + "DataCount.txt")
        do {
            try String(count).write(to: fileURL, atomically: true, encoding: .utf8)
        } catch {
            print("Error write")
        }
    }
